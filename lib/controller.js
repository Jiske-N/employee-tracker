  ...
  try {
    switch (selection) {
      ...
      case "Update an employee role":
        const { employeeName, updatedRole } = await enquiry(updateEmployee);
        console.log("blue", employeeName, updatedRole);
        const employeeId = await getEmployeeId(employeeName);
        const roleIdU = await getRoleId(updatedRole);
        console.log("mauve", employeeId, roleIdU);
