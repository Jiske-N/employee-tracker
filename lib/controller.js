  ...
  try {
    switch (selection) {
      ...
      case "View combined salaries of department":
        // select department from a list
        const { selectDepartmentSalary } = await enquiry(
          selectDepartmentSalaryP
        );

        // get selections id
        const departmentSalaryId = await getDepartmentId(
          selectDepartmentSalary
        );

        // retrieve an object containing the salaries from the database
        const salaries = await view2Q(departmentSalaryC, departmentSalaryId);

        // add the salaries from the object
        const totalSalary = salaries.reduce(
          (total, employee) => total + Number(employee.salary),
          0
        );

        // return message with total combined salary for department
        return `Total combined salaries for ${selectDepartmentSalary} are $${totalSalary}`;
