...
// query to view a table based on a second parameter
const view2Q = async function (input, id) {
  const result = await pool.query(input, [id]);
  return result.rows;
};
...
const updateEmployeeQ = async function (input, otherId, employeeId) {
  await pool.query(input, [otherId, employeeId]);
